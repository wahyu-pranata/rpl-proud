...
function goBack() {
    ...
    function addCard(sieValue, inputId) {
        ...
        card.className =
            "card border-2 rounded-lg p-4 border-gray-300 flex justify-between items-center shadow-md";
        card.setAttribute("data-input-id", inputId);

        card.innerHTML = `
        <div>
          <h3 class="font-bold text-gray-800 text-lg">Koordinator Sie ${sieValue}</h3>
          <p class="text-gray-500 text-sm">Undang mahasiswa untuk bergabung</p>
        </div>
        <button 
          class="text-blue-800 font-semibold border border-blue-800 px-4 py-2 rounded-lg hover:bg-blue-800 hover:text-white delete-card-btn">
          Undang
        </button>
      `;
